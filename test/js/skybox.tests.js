...
test("SELECT count() GROUP BY action_id ON :enter", function() {
  ...
  equal(html, 'Find the <span class="selection">number of actions performed</span> <span class="condition" data-condition-index="0">on session start</span>.')
});
...
test("SELECT count() GROUP BY action_id ON :enter AFTER <action>", function() {
  ...
  equal(html,
    'Find the <span class="selection">number of actions performed</span> ' +
