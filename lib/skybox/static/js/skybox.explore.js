  ...
  var node = g.node.selectAll(".node").data(nodes, function(d) { return d.key })
    .call(function(node) {
      ...
      rect.style("fill", nodeFillColor)
        ...
        .style("stroke-opacity", function(d) { return (d.depth == 0 && d.subdepth == 0 ? 1 : 0); })
        .call(flow.nodes.position)
...
skybox.explore.normalize = function(query, results, options) {
  ...
  if(query && results) {
    ...
    for(var selectionName in results) {
      var selectionNameArray = selectionName.split(":");
      var depth = parseInt(selectionNameArray[0]);
      var subdepth = parseInt(selectionNameArray[1]);
      var selection = selections[selectionName];
...
function appendToQuery(node, propertyName, withinRange) {
  ...
  var name = (withinRange[0] == 0 && withinRange[1] == 0 ? node.depth.toString() + ":" + (node.subdepth+1).toString() : (node.depth+1).toString() + ":0");
  selections = skybox.query.selections.hash(query);
  selection = selections[node.selectionName];
  ...
  condition.steps = [
    ...
    {type:"condition", expression:"true", within:withinRange, steps:[
      {type:"selection", name:name, dimensions:[propertyName], fields:[{name:"count", expression:"count()"}]}
    ]}    
  ];
